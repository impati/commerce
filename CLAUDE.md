...
- **커밋 단위가 유스케이스보다 작아지면 그 단위에 이름을 준다.** saga처럼 유스케이스를 트랜잭션으로 감쌀 수 없는 곳에서는 "함께 성립해야 하는 것"이 어디에도 얹혀 갈 수 없다. `XxxChanges`가 그 단위를 소유하고, 쓰기 포트를 그것만 쓰게 해서 규칙을 타입으로 막는다. **"함께 성립해야 한다"가 곧 "한 트랜잭션"은 아니다** — 수단은 참여자가 무엇을 공유하느냐가 정하며(트랜잭션·보상·멱등+재시도), 응용 계층은 `TransactionSection` 포트로 구간만 말한다. 상세는 [docs/architecture.md](docs/architecture.md), 근거는 [ADR-0012](docs/adr/0012-order-events-as-outbox.md).
...
- 2026-08-29 — 응용 계층에 `XxxChanges`(변경 단위)를 추가하고 `TransactionSection` 포트를 도입했다. 계기: BL-0052 리뷰에서 "주문 상태와 그 사건이 함께 성립한다"는 응집 단위에 소유자가 없다는 것이 드러났다. 지켜지던 이유는 JDBC 어댑터의 `save`가 우연히 둘 다 했기 때문이고, 그래야 한다고 말하는 곳이 없었다. order만 이런 이유는 saga라 커밋 단위가 유스케이스보다 작기 때문이며, 나머지 넷은 유스케이스 메서드에 트랜잭션을 걸어 해결하고 있다 (BL-0052).
