...
- 인터뷰에서 합의한 것만 ADR에 적는다. 진행 일지나 파일별 변경 목록은 ADR이 아니라 [CHANGELOG.md](CHANGELOG.md)에 간다.
...
| `CHANGELOG.md` | 무엇을 왜 바꿨나 — 의도와 결정 | 사람. 나중에 왜 이렇게 됐는지 물을 때 |
...
- **코드에 붙지 않는 후보는 [docs/backlog/](docs/backlog/)에 항목당 한 파일로 쓴다.** 배경과 목표만 적고 우선순위·상태·담당자는 항목 파일에 넣지 않는다. 순서는 [docs/backlog/README.md](docs/backlog/README.md)가 갖는다 — 순서는 바뀌고 항목은 남는다. 작업을 끝냈으면 항목 파일과 인덱스에서 지운다.
...
- **[CHANGELOG.md](CHANGELOG.md)는 변경 의도만 담는다.** 저장소를 바꾼 작업은 완료 전에 최상단에 한 항목을 추가한다. 파일별 변경 목록이나 완료한 일의 체크리스트는 쓰지 않는다 — 그건 `git log`가 정본이고 두 벌이 되면 어긋난다. 백로그 항목 추가는 기록 대상이 아니다.
