...
- 타입은 명시적으로 import한다. 본문에 전체 패키지명을 쓰거나 wildcard import를 사용하지 않는다. 같은 단순 이름의 타입이 충돌할 때만 예외를 둔다.
- 조건문은 한 문장이어도 중괄호와 줄바꿈을 사용한다. 생성자·접근자·콜백도 본문을 한 줄로 압축하지 않는다.
- 한 줄에 여러 문장을 배치하지 않는다. HTTP 호출 체인은 요청 구성·전송·응답 변환 단계가 보이도록 줄을 나눈다.
- 중첩 삼항식이나 람다에 조회·검증·실패 처리·변환을 함께 넣지 않는다. 처리 단계와 조건의 의도를 이름으로 드러낸다.
- 메서드 추출은 책임과 처리 단계가 기준이다. 클래스 분리는 응집과 의존성을 기준으로 판단한다.
- 테스트에도 같은 작성 기준을 적용한다. 긴 fixture는 준비와 검증의 의미를 읽을 수 있도록 배치한다.
...
- **쓰는 조회는 이름으로 드러낸다.** 작업 큐의 배타적 점유처럼 쓰기가 필요한 조회는 `find*`가 아니라 `claim*`으로 부른다. 위 룰이 막으려는 것은 조회로 보이는 것이 몰래 쓰는 상황이므로, 이름이 쓰기를 드러내면 그 오해가 생기지 않는다. 선례: [CheckoutProgressRepository.claim](apps/order-service/core/src/main/java/com/impati/commerce/order/application/port/out/CheckoutProgressRepository.java) ([ADR-0018](docs/adr/0018-durable-checkout-recovery.md))
...
- **비게이트웨이 HTTP 호출은 `ServiceCallExecutor`로 조회와 명령을 선언한다.** 호출자 계약으로 인정한 4xx만 어댑터가 구조화된 오류 코드로 번역하고, 나머지 응답·전송 실패는 공통 경계가 `downstream_error`·`service_unavailable`·`outcome_unknown`으로 구분한다. 다운스트림 메시지는 로그에만 남기고 공개 메시지는 호출자가 소유한다. 게이트웨이는 상태와 본문을 그대로 전달하는 프록시이므로 예외다 (ADR-0021).
- 서비스 간 HTTP 클라이언트는 [RestClientFactory](libs/common-http/src/main/java/com/impati/commerce/http/RestClientFactory.java)로 만든다. 어댑터가 `RestClient.Builder`를 직접 주입받으면 빌더 단위로 동작하는 테스트 스텁과 공통 커스터마이저가 갈라진다. 서비스 간 호출이 있는 실행 단위는 `RestClientAutoConfiguration`을 제외해 직접 주입을 막는다 (ADR-0019).
...
- 2026-09-17 — 명시적 import, 블록 표현, 처리 단계와 테스트 가독성 기준을 추가했다. 계기: BL-0082에서 기존 코드의 작성 방식을 따르지 않고 조회·실패 처리·변환을 압축한 부분을 사용자 리뷰로 확인했다.
