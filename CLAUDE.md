...
- **비즈니스 규칙은 [docs/policy/](docs/policy/)에 남긴다.** 사람이 정해야 하는 것 — 무엇을 거절하고 무엇을 허용하나 — 이 대상이고, 규칙마다 `PD-NNNN-RN` ID를 붙여 테스트 주석이 그 규칙을 가리키게 한다. **참조는 한 방향이다** — 코드가 정책을 가리키고 정책 문서는 코드를 가리키지 않는다. 지금 무엇이 참인지는 테스트가 답하므로 **"유효한 규칙 전부"를 모은 문서를 만들지 않는다.** 규약과 경계는 [docs/policy/README.md](docs/policy/README.md)에 있다.
- **아직 병합되지 않은 작업에서 만든 정책·ADR은 대체하지 않고 고친다.** 대체 규약은 `main`에 들어간 문서부터 적용한다. 같은 브랜치 안에서 대체하면 한 번도 참이었던 적 없는 문서가 목록에 남고, 그 이력은 이미 브랜치의 `git log`가 갖는다.
