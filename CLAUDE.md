...
- **비즈니스 규칙은 [docs/policy/](docs/policy/)에 남긴다.** 사람이 정해야 하는 것 — 무엇을 거절하고 무엇을 허용하나 — 이 대상이고, 규칙마다 `PD-NNNN-RN` ID를 붙여 테스트 주석이 그 규칙을 가리키게 한다. 지금 무엇이 참인지는 테스트가 답하므로 **"유효한 규칙 전부"를 모은 문서를 만들지 않는다.** 규약과 경계는 [docs/policy/README.md](docs/policy/README.md)에 있다.
