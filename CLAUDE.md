...
**작업을 이어서 시작할 때는 [NEXT.md](NEXT.md)를 먼저 읽는다.** 우선순위, 합의된 방향, 미결정 사항이 거기 있다.
...
| `decisions/` | 기술 결정 — 무엇을 왜 골랐나, 언제 다시 보나 | 사람. 결정을 뒤집을 때 |
| `problem/` | 겪은 문제와 기술 조사 | 사람. **사용자가 요구할 때만 쓴다** |
| `NEXT.md` | 이어서 할 일 — 우선순위·합의·미결정 | 다음 세션의 시작점 |
...
- **부채는 코드의 `TODO` 주석에 남긴다.** 코드와 어긋날 수 없고 코드를 지우면 함께 사라진다. `make todo`로 센다. 코드에 붙지 않는 것(우선순위, 합의, 미결정)만 [NEXT.md](NEXT.md)에 쓴다.
- **완료된 일 목록을 문서로 만들지 않는다.** `git log`가 정본이며 두 벌이 되면 어긋난다. 작업을 끝냈으면 NEXT.md에서 해당 항목을 지운다.
- **기술 결정은 [decisions/](decisions/)에 남긴다.** 채택하지 않은 대안과 그 이유, 그리고 재검토 조건까지 적는다. 코드를 읽어도 알 수 없는 것이 그 셋이다. 결정을 뒤집을 때는 기존 문서를 고치지 말고 새 번호로 대체한다 — 고치면 왜 뒤집었는지가 사라진다.
