...
| 리뷰한다 | `rca-code-review` | 채팅 보고만. 사용자가 요청할 때만 돈다 |
...
- **완료한 백로그 항목은 지우지 말고 [docs/backlog/done/](docs/backlog/done/)으로 `git mv` 한다.** 상태는 위치로만 구분한다. 완료 기준 검증을 통과한 뒤 한 번만 옮기고, 그 이동을 마지막 커밋에 포함한다. 검증에 실패했거나 중단했으면 옮기지 않는다. **`BL-NNNN` 채번은 `done/`까지 세서** 번호를 재사용하지 않는다 — 지우면 다음 번호를 잘못 딴다.
