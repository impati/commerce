...
class StorefrontRoutingTest {
    ...
    void forwardsQuantityAndExpectedVersionUsingTheAuthenticatedMember() throws Exception {
        ...
        server.expect(requestTo("http://localhost:8110/cart/items/sku"))
                ...
                .andRespond(withSuccess("""
                        ...
                        """, MediaType.APPLICATION_JSON));
        mvc.perform(put("/cart/items/sku").header("Authorization", "Bearer access").header("X-Member-Id", "someone_else")
    ...
    void forwardsDeletionAndPreservesTheVersionConflict() throws Exception {
        server.expect(requestTo("http://localhost:8110/cart/items/sku?expectedVersion=5"))
                .andExpect(method(HttpMethod.DELETE))
                .andExpect(header("X-Member-Id", "m"))
                .andRespond(withStatus(HttpStatus.CONFLICT).contentType(MediaType.APPLICATION_JSON)
                        .body("{\"code\":\"cart_changed\",\"message\":\"Reload\"}"));
        mvc.perform(delete("/cart/items/sku").header("Authorization", "Bearer access").header("X-Member-Id", "someone_else").param("expectedVersion", "5"))
                .andExpect(status().isConflict())
