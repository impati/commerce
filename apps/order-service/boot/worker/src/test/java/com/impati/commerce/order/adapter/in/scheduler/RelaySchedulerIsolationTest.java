...
/**
 * 릴레이가 전용 스케줄러에서 돌고 체크아웃 복구는 기본에 남는지 확인한다 (ADR-0017, ADR-0018).
 *
...
class RelaySchedulerIsolationTest {
    ...
    /**
     * 체크아웃 복구는 스케줄러를 지정하지 않아 기본에 남는다.
     *
     * <p>릴레이만 옮기고 정리는 그대로 두는 것이 격리의 요점이다. 정리까지 전용 빈으로 옮기면
     * 무엇으로부터 격리한 것인지 사라진다.
     */
    @Test
    void checkoutRecoveryStaysOnTheDefaultScheduler() throws NoSuchMethodException {
        var scheduled = CheckoutRecoveryScheduler.class
                .getDeclaredMethod("recover").getAnnotation(Scheduled.class);

        assertThat(scheduled.scheduler()).isEmpty();
        assertThat(scheduled.fixedDelayString()).isEqualTo("${orders.checkout-recovery-period-ms:10000}");
    }
