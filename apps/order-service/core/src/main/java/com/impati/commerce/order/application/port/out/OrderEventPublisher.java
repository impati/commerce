 ...
 * 브로커에서 조용히 깨진다.
 *
 * <p><b>같은 주문의 사건은 일어난 순서로 나간다</b> (ADR-0016). 부르는 쪽이 한 주문을 통째로
 * 점유해 순서대로 넘기고, 한 건이 실패하면 같은 주문의 뒤 건을 그 주기에서 보내지 않는다.
 * 구현은 {@link OrderEvent#partitionKey()}가 같은 것들이 같은 순서로 소비되게 해야 한다.
 *
 * <p><b>주문이 다르면 순서를 보장하지 않는다.</b> 서로 다른 주문의 사건은 다른 인스턴스가 동시에
 * 보낼 수 있다. 지키지 못하는 것을 계약에 적어두면 그 문장을 믿고 짠 소비자가 나중에 틀린다.
 *
