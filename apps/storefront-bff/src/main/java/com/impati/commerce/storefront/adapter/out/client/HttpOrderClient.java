...
public class HttpOrderClient implements OrderClient {
    ...
    public PurchaseQuoteResponse quote(String memberId, long version) {
        return calls.query("storefront purchase quote", () -> restClient.get().uri(builder -> builder.path("/internal/purchase-quotes").queryParam("cartVersion", version).build())
            ...
            if (error.hasCode("cart_changed")) throw DomainException.cartChanged("장바구니가 변경됐습니다. 다시 확인해주세요.");
        });
    }
    public org.springframework.http.ResponseEntity<CheckoutResponse> checkout(String memberId, String key, ConfirmedCheckoutRequest request) {
        return calls.command("storefront confirmed checkout", () -> restClient.post().uri("/checkouts/confirmed")
            ...
            if (error.hasCode("quote_changed")) throw new DomainException("quote_changed", "구매 내용이나 금액이 변경됐습니다. 새 견적을 확인해주세요.", 409);
            if (error.hasCode("cart_changed")) throw DomainException.cartChanged("장바구니가 변경됐습니다. 다시 확인해주세요.");
            if (error.hasCode("cart_empty")) throw DomainException.cartEmpty("장바구니가 비어 있습니다.");
            if (error.hasCode("validation_error")) throw DomainException.validation("주문 요청을 확인해주세요.");
            if (error.hasCode("conflict")) throw DomainException.conflict("같은 주문 요청의 내용이 다릅니다.");
            if (error.hasCode("not_found")) throw DomainException.notFound("주문 정보를 찾을 수 없습니다.");
        });
    }
}
