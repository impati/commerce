...
public class CartPageExecutor implements CartPageUseCase {
    ...
    public CartPage get(String memberId) {
        ...
        var stockResult = stocks.stream().map(CompletableFuture::join).filter(Objects::nonNull).toList();
        var quoteResult = quote.join();
        // Order가 조회한 버전과 줄이 다른 응답은 부분 합계로 사용하지 않는다.
        if (quoteResult != null && (quoteResult.cartVersion() != cart.version() || !sameLines(cart, quoteResult))) {
            quoteResult = null;
        }
        ...
        for (int i = 0; i < cart.lines().size(); i++) {
            ...
            if (description == null) {
                failures.add("product:" + line.skuId());
            }
            if (quantity == null) {
                failures.add("inventory:" + line.skuId());
            }
        ...
        var converted = quoteResult == null ? null : new CartPage.Quote(quoteResult.id(), quoteResult.cartVersion(),
                quoteResult.lines().stream().map(line -> new CartPage.PriceLine(line.skuId(), line.quantity(), line.unitPrice(), line.lineTotal()))
                        .toList(), quoteResult.total());
        boolean canCheckout =
                converted != null && lines.stream().allMatch(line -> line.availableQuantity() != null && line.availableQuantity() >= line.quantity());
