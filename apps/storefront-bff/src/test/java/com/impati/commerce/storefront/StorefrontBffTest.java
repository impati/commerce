...
class StorefrontBffTest {
    ...
    void rendersPartialFailureInsteadOfAFakeCartOrPrice() throws Exception {
        ...
        server.expect(requestTo("http://localhost:8104/internal/stock/sku")).andRespond(withServerError());
        server.expect(requestTo("http://localhost:8108/internal/purchase-quotes?cartVersion=5")).andRespond(withServerError());
