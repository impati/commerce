...
**유스케이스 인터페이스(`port/in`)는 두지 않습니다.** 시험해봤고 값이 나오지 않아 걷어냈습니다. 들어오는 쪽은 어댑터가 응용을 부르므로 의존 방향이 이미 맞아 뒤집을 것이 없고, 남는 값은 "소비자가 자기가 쓰는 것만 안다"인데 **소비자가 하나면 그 값이 0**입니다. payment-service는 컨트롤러 하나가 다섯 동작을 전부 쓰므로, 나눴더니 같은 빈이 다섯 번 주입되고 생성자만 길어졌습니다.

도입 조건은 **소비자가 갈릴 때**입니다. 인바운드 어댑터가 둘 이상이거나(웹 + 스케줄러 + 메시지 소비자) 컨트롤러가 여럿이라 각자 다른 부분집합을 쓸 때입니다. 그때 뽑아도 비용은 인터페이스 추출과 타입 변경뿐이라, 미리 만들어두는 것이 벌어주는 것이 없습니다.
