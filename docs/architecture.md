...
| 구현 | `XxxExecutor` (`@Component`) | 유스케이스의 흐름을 조율합니다. 결정은 도메인에 위임하고 불변식을 갖지 않습니다 — 응용 계층에 있는 불변식은 강제되지 않으므로, 애그리거트로 지킬 수 없어 다른 강제 수단(DB 제약)을 함께 둘 때만 여기 둡니다 |
| 변경 단위 | `XxxChanges` (`@Component`) | 한 애그리거트의 변경을 확정합니다. **함께 성립해야 하는 것을 소유합니다** (아래) |
| 매퍼 | `XxxMapper` | 도메인 ↔ 유스케이스 입출력. **package-private입니다** |
| 구동자 | — | 스케줄러나 앱 시작이 트리거인 진입점. **여기 있어서는 안 됩니다** (아래) |

### 변경 단위 (`XxxChanges`)

**유스케이스가 곧 커밋 단위인 서비스에는 필요 없습니다.** 그런 곳은 유스케이스 메서드에 트랜잭션을 걸면 끝나고, 실제로 order 외의 서비스가 전부 그렇습니다.

**saga가 있으면 달라집니다.** `checkout`은 외부 호출을 품고 있어 메서드를 트랜잭션으로 감쌀 수 없습니다 — 감싸면 DB 커넥션을 물고 결제·배송 응답을 기다립니다. 그래서 커밋 단위가 유스케이스보다 작고, **작아진 그 단위가 어딘가에 얹혀 갈 수 없으므로 자기 이름을 가져야 합니다.**

`OrderChanges`가 지키는 것은 하나입니다 — 주문의 상태와 그 상태를 만든 사건은 함께 성립합니다. 그리고 **실행자는 이것 말고 주문을 저장할 방법이 없습니다.** 쓰기 포트(`OrderWriter`)를 여기서만 쓰는 것이 그 게이트이고, 그래서 사건 없이 저장하는 코드가 컴파일되지 않습니다. 규칙을 문장으로 적어두면 새고 타입으로 막으면 새지 않습니다.

**"함께 성립해야 한다"가 곧 "한 트랜잭션"은 아닙니다.** 이 저장소에 수단이 셋 있고, 무엇을 쓸지는 참여자가 무엇을 공유하느냐가 정합니다.

| 수단 | 예 | 쓰는 조건 |
| --- | --- | --- |
| 트랜잭션 | 주문과 그 사건 | 같은 내구 저장소를 공유할 때 |
| 보상 | checkout saga | 저장소가 갈라져 있고 되돌릴 수 있을 때 |
| 멱등 + 재시도 | 알림 발송 | 되돌릴 수 없고 반복이 안전할 때 |

그래서 응용 계층은 `TransactionSection` 포트로 **"이 구간은 함께 성립해야 한다"만 말하고**, 그것을 무엇으로 지킬지는 어댑터가 정합니다. 근거는 [ADR-0012](adr/0012-order-events-as-outbox.md).
