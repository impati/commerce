...
수량 변경·삭제 요청은 브라우저에서 조회한 예상 버전을 Cart까지 전달하고 현재 버전과 정확히 같아야 허용한다. Cart의 버전은 읽은 상태를 저장할 때도 비교하고, 다른 쓰기나 구매분 분리가 먼저 진행됐으면 오래된 저장을 거절한다. 장바구니의 헤더와 줄 조회는 같은 DB 스냅샷으로 읽는다. ([ADR-0023](adr/0023-storefront-bff.md), [ADR-0024](adr/0024-cart-line-management.md), [PD-0021](policy/pd-0021-purchase-confirmation.md))
