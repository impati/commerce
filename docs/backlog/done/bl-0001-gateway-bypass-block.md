...
[BL-0012](bl-0012-split-runtime-modules.md)의 member external/internal 분리가 이 목표의 대안 해법이기도 하다 — 공유 시크릿은 코드로 막는 것이고 별도 프로세스는 네트워크로 막는 것이라 후자가 강하다. 같은 일을 두 번 하지 않도록 함께 판단한다.
