...
원인은 **요청의 문제와 우리 쪽의 문제를 구분할 자리가 없다는 것**이다. [BL-0067](bl-0067-preserve-downstream-failure-semantics.md)이 형제 서비스 호출에서 지적한 것과 같은 계열이며, 그쪽은 `conflict`로 새고 여기서는 `unauthorized`로 샌다.
