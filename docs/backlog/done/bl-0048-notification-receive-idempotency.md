...
지금은 그 분업이 없다. 발신측이 재시도를 붙이는 만큼 중복이 그대로 사용자에게 간다. [BL-0004](bl-0004-member-service-outbox.md)가 member-service에 재시도를 붙이면서 이 구멍이 실제로 열렸고, [BL-0046](bl-0046-order-notification-loss.md)이 order-service에 붙이면 두 배가 된다.
